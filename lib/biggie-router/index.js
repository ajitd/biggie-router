...
Router.prototype._modifyRequest = function _modifyRequest(request, response) {
  ...
  var getDefaultHeaders,
      self = this;

  getDefaultHeaders = function getDefaultHeaders(mime) {
    var headers = {};
    Object.keys(self.config.headers).forEach(function (key) {
      headers[key] = self.config.headers[key];
    });
    if (mime) {
  ...
  response.sendBody = function sendBody(code, content, headers) {
    ...
    var default_headers = getDefaultHeaders();
    headers = headers || {};
  ...
  response.sendJson = function sendJson(code, data, headers) {
    ...
    var default_headers = getDefaultHeaders('application/json');
    headers = headers || {};
  ...
  response.sendText = function sendText(code, data, headers) {
    ...
    var default_headers = getDefaultHeaders('text/plain');
    headers = headers || {};
